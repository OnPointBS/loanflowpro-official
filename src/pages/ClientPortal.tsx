...
const ClientPortal: React.FC = () => {
  ...
  // Fallback client name for preview mode
  const previewClientName = React.useMemo(() => {
    if (isPreviewMode) {
      // Try to get the client name from the URL parameter
      if (clientIdFromUrl) {
        // Extract a readable name from the client ID
        if (clientIdFromUrl === 'client1') return 'John Smith';
        if (clientIdFromUrl === 'client2') return 'Sarah Johnson';
        if (clientIdFromUrl === 'client3') return 'Mike Davis';
  ...
  const isDataLoading = !user || !workspace || !currentWorkspace || !currentMembership;

  if (isDataLoading) {
    console.log('🔍 Loading state: waiting for data to load');
    return (
  ...
  const displayLoanTypes = isPreviewMode ? sampleLoanTypes : [];
  const displayTasks = isPreviewMode ? sampleTasks : [];

  // Simplified permission checks for preview mode
  const canViewLoanFiles = isPreviewMode;
  const canViewDocuments = isPreviewMode;
  const canViewTasks = isPreviewMode;
  const canViewAnalytics = isPreviewMode;
  const canUploadDocuments = isPreviewMode;
  const canSendMessages = isPreviewMode;
  ...
  const handleFileUpload = async () => {
    ...
    setUploading(true);
    try {
      // For preview mode, we don't have real loan files
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              <h1 className="text-3xl font-bold text-gunmetal">
                {isPreviewMode ? `Client Portal - ${primaryAdvisor?.user?.name || previewClientName || 'Preview Mode'}` : 'Client Portal'}
              </h1>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
                  ...
                  {canViewDocuments && (
                    <div className="text-center p-4 bg-brand-orange/10 rounded-lg">
                      ...
                      <p className="text-sm text-gunmetal-light">{isPreviewMode ? '0' : '0'} uploaded</p>
                    </div>
                  )}
                  {canViewTasks && (
                    <div className="text-center p-4 bg-brand-orange/10 rounded-lg">
                ...
                <div className="space-y-3">
                  {isPreviewMode ? (
                    <div className="text-center p-4 bg-gray-50 rounded-lg">
                      <FileText className="w-8 h-8 text-brand-orange mx-auto mb-2" />
                      <p className="font-medium text-gunmetal">Sample Loan Activity</p>
                      <p className="text-sm text-gunmetal-light">Preview mode - no real data</p>
                    </div>
                  ) : (
